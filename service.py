...
def init_db():
    ...
    # user_time(작업자가 폼에서 선택한 시간)을 in_t(출입) / out_t(퇴실)에 저장
...
def save_to_db(mode, type_, proj, name, phone, task_reason, time_val, health, photo_path):
    ...
    cur = conn.cursor()
    # 날짜는 실제 시스템 수신 날짜 기준 (새벽 6시 기준일 변경 적용 시 조절 가능)
    today_date = datetime.now().strftime("%Y-%m-%d")
    
    if mode == "상시": 
        # 상시는 꼬리표 추적을 위해 무조건 새 줄(INSERT)
        if type_ == "퇴실":
        ...
        # 작업 모드는 당일 동일 인물 1줄로 병합(UPDATE) 처리
...
def queue_worker():
    while True:
        try:
            ...
            # MMS인 경우 기기에 사진이 완전히 다운로드 될 때까지 3초 안전 대기
            if action == "mms": sleep(3) 
                
            try:
                # 💡 정규식(Regex)을 이용한 무결점 파싱 로직
...
class MasterReceiver(PythonJavaClass):
    ...
    def onReceive(self, ctx, intent):
        ...
        if action == "android.provider.Telephony.SMS_RECEIVED":
            ...
            msg_type = "mms"
            body = "SYS_QR_" # MMS 큐 진입을 위한 강제 우회 태그
